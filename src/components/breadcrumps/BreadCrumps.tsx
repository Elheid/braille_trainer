            ...
            if (paths.includes("result")) setBreadcrumbs(
                ...
                for (const path of paths) {
                    ...
                    if (categoryId || subCategoryId) {
                        if (categoryId) title =categoryId// getCategoryTitleById(Number(categoryId));//await getCategoryNameById(Number(categoryId));
                        if (subCategoryId) title = subCategoryIdFromUrl//getCategoryTitleById(Number(subCategoryId))//`Подкатегория ${subCategoryId}`;
                    }

                    // Добавляем хлебные крошки для каждого сегмента
                    newBreadcrumbs.push({
                        destination: decodeURIComponent(path) === "subCategories" ? "/services" : destination,
                        content: decodeURIComponent(path) === "learning" ? "Изучение":  decodeURIComponent(path) === "training" ? 'Тренировка' : title
                    });
                };
        ...
        <div ref={breadcrumbContainerRef}>
            ...
            <BreadCrumpComponent  destination={"/services"} class={"current-page"} content={"Выбор категорий"}/>*/}
                {breadcrumbs.map((breadcrumb, index) => (
                    <div key={index} className={`${styles["breadcrumb-item"]} page`}>
